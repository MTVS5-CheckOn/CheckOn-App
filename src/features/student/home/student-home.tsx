...
function WorksheetCard({ item }: { item: WorksheetSummary }) {
  return <Link href={`${ROUTES.student.worksheets}/${item.id}`} className="block rounded-card focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-action"><Card className="flex min-h-[108px] items-center gap-3 p-4"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><Badge>{item.area}</Badge><Badge tone={item.status === "신규" ? "warning" : "info"}>{item.status}</Badge></div><h3 className="mt-2 truncate text-[15px] font-semibold">{item.title}</h3><p className="mt-1 text-xs text-subtle">{item.meta}</p></div><ChevronRight aria-hidden size={18} className="shrink-0 text-[#9AA8BC]" /></Card></Link>;
}

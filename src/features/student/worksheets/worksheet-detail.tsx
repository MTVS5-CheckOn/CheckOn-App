...
export function WorksheetDetail({ worksheetId }: { worksheetId: string }) {
  const { data: worksheet, isLoading } = useWorksheetQuery(worksheetId);
  if (isLoading) return <p className="p-5 text-sm text-muted">학습지를 불러오고 있어요.</p>;

...
export function QuizQuestionForm({ worksheetId }: { worksheetId: string }) {
  ...
  const question = data?.questions[currentIndex] ?? data?.questions[0];
  ...
  if (isError || !worksheet || !question) return <div className="p-8 text-center"><p className="text-sm font-bold">문제 정보를 불러오지 못했어요.</p><button onClick={() => refetch()} className="mt-4 rounded-xl bg-brand px-5 py-2 text-sm font-bold">다시 시도</button></div>;
  const submit = async () => { const value = content.trim(); if (value.length < 5) return; try { const created = await createQuestion.mutateAsync({ worksheetId, worksheetTitle: worksheet.title, questionNumber: currentIndex + 1, content: value }); router.push(routeBuilders.student.questionComplete(created.id, routeBuilders.student.solveWorksheet(worksheetId))); } catch { return; } };

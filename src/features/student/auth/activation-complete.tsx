...
export function ActivationComplete() {
  ...
  const { data } = useStudentActivationQuery();
  return (
    <div className="flex min-h-dvh flex-col bg-app">
      ...
      <main className="flex flex-1 flex-col items-center justify-center px-5 pb-20 text-center">
        ...
        <p className="mt-3 text-sm leading-6 text-muted">{data?.name ?? name} · {data?.grade ?? grade}<br />학부모 자녀 등록이 완료되어 모든 학습 기능이 열렸습니다.</p>

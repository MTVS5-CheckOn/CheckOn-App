...
import { GRADE_TO_CONTRACT_VALUE, STUDENT_GRADES, studentSignupSchema, type StudentSignupValues } from "@/features/student/auth/schema";
...
export function SignupInfoForm() {
  ...
  const canSubmit = Boolean((values.name?.trim().length ?? 0) >= 2 && values.email && values.grade && (values.password?.length ?? 0) >= 8 && values.password === values.passwordConfirm);
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    // 🔴 계약의 grade 는 정수 1~3 이다. "N수생" 에 대응하는 값이 없어 임의로 채우지 않는다.
    const gradeValue = GRADE_TO_CONTRACT_VALUE[result.data.grade];
    if (gradeValue === null) {
      setError("grade", { message: "N수생 가입은 아직 지원하지 않습니다. 백엔드 학년 값이 고1~고3만 받습니다." });
      return;
    }
    try {
    ...
    } catch { setError("root", { message: "회원가입을 완료하지 못했습니다. 다시 시도해 주세요." }); }
  });
  ...
  return (
    <form onSubmit={onSubmit} className="flex min-h-dvh flex-col bg-app">
      ...
      <main className="flex-1 space-y-4 px-5 py-4">
        ...
        <FormField label="이메일" type="email" placeholder="student@example.com" error={errors.email?.message} {...register("email")} />
        <fieldset>
          <legend className="mb-1.5 text-sm font-bold">학년</legend>
          <div className="grid grid-cols-4 gap-2">
            {STUDENT_GRADES.map((grade) => <button key={grade} type="button" onClick={() => setValue("grade", grade, { shouldValidate: true })} className={`h-11 rounded-xl border text-sm font-semibold ${selectedGrade === grade ? "border-brand bg-[#FFF0E5] text-[#B5572D]" : "border-border bg-surface text-muted"}`}>{grade}</button>)}
          </div>
          {errors.grade ? <p className="mt-1.5 text-xs text-[#D64545]">{errors.grade.message}</p> : null}
        </fieldset>
        <FormField label="비밀번호" type={showPassword ? "text" : "password"} placeholder="8자 이상 입력" error={errors.password?.message} trailing={<button type="button" className="grid size-8 place-items-center text-subtle" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? "비밀번호 숨기기" : "비밀번호 보기"}>{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button>} {...register("password")} />

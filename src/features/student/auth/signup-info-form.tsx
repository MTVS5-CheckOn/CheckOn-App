...
export function SignupInfoForm() {
  ...
  return (
    <form onSubmit={onSubmit} className="flex min-h-dvh flex-col bg-app">
      ...
      <div className="border-t border-divider bg-surface px-5 pb-[calc(20px+env(safe-area-inset-bottom))] pt-3"><ActionButton type="submit" disabled={!canSubmit || isSubmitting}>가입 완료</ActionButton></div>
    </form>
  );
}

...
export function ParentAnalysis() {
  ...
  const { percentile, accuracy, changeFromPreviousMonth, sampleAsOf, areaScores, accuracyTrend } = data;
  return <div className="space-y-4 px-5 py-4">
    ...
    <Link href={routeBuilders.parent.weakness("독서")} className="flex items-center rounded-card border border-border bg-surface p-4"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="rounded bg-[#E85A4F] px-1.5 py-1 text-[11px] font-bold text-white">1순위 취약</span><strong className="text-sm">독서 · 개념/지식</strong></div><p className="mt-2 text-xs text-muted">정답률 43% · 최근 4주 집중 보완 필요</p><p className="mt-1 text-[11px] text-subtle">전국 동일 학년 표본 · 2026.08.31 기준</p></div><ChevronRight className="text-action" size={18} /></Link>
...
export function WeaknessDetail({ area }: { area: string }) {
  ...
  const { areaScores } = data;
  return <div className="space-y-4 px-5 py-4">
    <section className="rounded-card border border-border bg-surface p-4"><span className="rounded bg-[#E85A4F] px-1.5 py-1 text-[11px] font-bold text-white">1순위 취약</span><h2 className="mt-3 text-xl font-bold">{area}</h2><p className="mt-2 text-sm text-muted">개념/지식 유형 · 정답률 <strong className="text-[#E85A4F]">43%</strong></p></section>
    ...
    <section className="rounded-card border border-border bg-surface p-4"><h3 className="text-sm font-bold text-muted">최근 4주 학습 패턴</h3><dl className="mt-3 divide-y divide-divider text-sm"><Row label="학습 빈도" value="주 1~2회 (비중 낮음)" /><Row label="평균 풀이 시간" value="문항당 3.2분 (전체 평균 2.1분)" /><Row label="근거 문항 수" value="12문항" /></dl></section>
    <Link href={routeBuilders.parent.record("l3")} className="flex h-[52px] items-center justify-center rounded-xl border border-[#A9D4F2] bg-surface text-sm font-bold text-[#2F6FA7]">관련 학습기록 보기</Link>
    <div className="flex gap-2 rounded-xl bg-warning-soft p-3 text-xs leading-5 text-[#7C6210]"><AlertTriangle size={17} className="mt-0.5 shrink-0" /><p>백분위와 취약 영역은 동일 학년의 익명 학습 표본을 기준으로 산출됩니다.</p></div>
  </div>;
}

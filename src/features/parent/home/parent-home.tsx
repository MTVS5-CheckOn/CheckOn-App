...
export function ParentHome() {
  ...
  return <div className="space-y-4 p-5">
    <section className="rounded-lg bg-brand p-5 text-[#4C3024]"><p className="text-xs opacity-70">이번 달 학습 현황</p><h1 className="mt-1 text-xl font-bold">{data.student.name ?? child?.name ?? "자녀를 등록해 주세요"}</h1><p className="mt-1 text-xs opacity-70">{data.student.period}</p></section>
    ...
    <Link href={ROUTES.parent.consultations} className="block"><Card className="flex items-center gap-3 p-4"><span className="grid size-11 shrink-0 place-items-center rounded-full bg-brand-soft text-[#D96534]"><MessageCircle size={20} /></span><div className="min-w-0 flex-1"><h2 className="text-sm font-bold">선생님과 상담이 필요하신가요?</h2><p className="mt-1 text-xs leading-5 text-muted">학습 방향을 상담하고 앱으로 답변받을 수 있어요.</p></div><ChevronRight aria-hidden size={18} className="text-action" /></Card></Link>
    ...
    {selectorOpen ? <div className="fixed inset-0 z-50 flex justify-center bg-[#202939]/35 px-5 pt-[76px]" onClick={() => setSelectorOpen(false)}><section role="dialog" aria-modal="true" aria-label="자녀 선택" onClick={(event) => event.stopPropagation()} className="h-fit w-full max-w-[350px] rounded-card border border-border bg-surface p-4 shadow-xl"><div className="flex items-center justify-between"><p className="text-xs text-muted">학습 현황을 확인할 자녀</p><button onClick={() => setSelectorOpen(false)} aria-label="닫기" className="grid size-9 place-items-center"><X size={18} /></button></div>{children.map((item) => <button key={item.studentId} onClick={() => { selectChild(item.studentId); setSelectorOpen(false); }} className="mt-1 flex min-h-14 w-full items-center border-b border-divider text-left last:border-0"><span className="flex-1 font-bold">{item.name} · {item.grade}</span>{selectedChildId === item.studentId ? <Check size={19} className="text-[#26856B]" /> : null}</button>)}<Link href="/parent/profile/children/new" className="mt-2 flex min-h-12 items-center gap-2 font-semibold text-action"><Plus size={18} />자녀 추가 등록</Link></section></div> : null}
  </div>;
}

...
export function ParentProfile() {
  ...
  return (
    <div className="space-y-7 px-5 py-4">
      ...
      <ProfileSection title="등록된 자녀"><div className="overflow-hidden rounded-card border border-border bg-surface">{children.map((child) => <div key={child.id} className="flex min-h-[72px] items-center gap-3 border-b border-divider px-4"><span className="grid size-10 place-items-center rounded-full bg-brand font-bold text-[#7D452C]">{child.name.slice(0, 1)}</span><div className="min-w-0 flex-1"><p className="font-bold">{child.name}</p><p className="text-xs text-muted">{child.grade} · {child.studentId} · {child.active ? "활성" : "비활성"}</p></div><span className="rounded bg-[#E8F6F1] px-2 py-1 text-[11px] font-bold text-[#26856B]">연결됨</span></div>)}<Link href={routeBuilders.parent.childRegister()} className="flex min-h-[60px] items-center gap-3 px-4 text-sm font-semibold text-action"><PlusCircle />자녀 등록하기</Link></div></ProfileSection>

      <ProfileSection title="연결된 강사"><div className="overflow-hidden rounded-card border border-border bg-surface">{teachers.map((teacher) => <div key={teacher.id} className="flex min-h-[72px] items-center gap-3 border-b border-divider px-4"><span className="grid size-10 place-items-center rounded-full bg-[#EEF4FF] font-bold text-action">{teacher.name.slice(0, 1)}</span><div><p className="font-bold">{teacher.name}</p><p className="text-xs text-muted">{teacher.academy}</p></div></div>)}<Link href={routeBuilders.parent.inviteCode()} className="flex min-h-[60px] items-center gap-3 px-4 text-sm font-semibold text-action"><PlusCircle />초대 코드 등록하기</Link></div></ProfileSection>

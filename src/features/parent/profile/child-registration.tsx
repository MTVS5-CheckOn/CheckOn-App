...
export function ChildRegistration() {
  ...
  // 🔴 사전 확인은 200 + registrable 로 답한다. 등록 불가를 오류로 던지지 않으므로 여기서 갈라야 한다.
  async function verify() {
    try {
      const result = await verifyMutation.mutateAsync(studentId.trim().toUpperCase());
      if (!result.registrable) {
        setError(result.reason === "ALREADY_LINKED" ? "이미 다른 학부모 계정에 등록된 학생입니다." : "학생 정보를 찾을 수 없습니다.");
        return;
      }
      setError("");
      setStep("confirm");
    } catch (caught) { setError(caught instanceof Error ? caught.message : "학생 정보를 확인하지 못했습니다."); }
  }
  async function register() { try { const child = await registerMutation.mutateAsync({ studentPublicId: studentId.trim().toUpperCase() }); addChild(child); setStep("complete"); } catch (caught) { setError(caught instanceof Error ? caught.message : "자녀를 등록하지 못했습니다."); setStep("input"); } }
  ...
  if (step === "confirm") return <div className="space-y-4 p-5"><section className="rounded-card border border-border bg-surface p-5"><div className="flex items-center gap-3"><span className="grid size-12 place-items-center rounded-full bg-brand"><UserRound /></span><div><p className="text-xs text-muted">등록할 학생</p><h2 className="mt-1 text-lg font-bold">{verifyMutation.data?.name ?? "확인된 학생"} · {verifyMutation.data?.grade == null ? "" : `고${verifyMutation.data.grade}`}</h2><p className="text-xs text-muted">{studentId.trim().toUpperCase()} · 현재 비활성</p></div></div></section><div className="flex gap-2 rounded-xl bg-warning-soft p-3 text-xs leading-5 text-[#7C6210]"><Info size={17} className="shrink-0" />등록하면 학생 계정이 즉시 활성화되며 다른 학부모 계정에는 등록할 수 없습니다.</div><ActionButton disabled={registerMutation.isPending} onClick={register}>{registerMutation.isPending ? "등록 중..." : "이 학생을 자녀로 등록"}</ActionButton><ActionButton variant="secondary" onClick={() => setStep("input")}>다시 확인하기</ActionButton></div>;

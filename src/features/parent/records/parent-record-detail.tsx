...
export function ParentRecordDetail({ recordId }: { recordId: string }) {
  ...
  return <div className="space-y-4 px-5 py-4">
    ...
    <section className="rounded-card border border-border bg-surface p-4"><h3 className="text-sm font-bold text-muted">정답률 추이</h3><div className="mt-2 h-40" aria-label="정답률 추이 선 차트"><ResponsiveContainer width="100%" height="100%"><LineChart data={accuracyTrend} margin={{ top: 12, right: 8, bottom: 0, left: -26 }}><CartesianGrid stroke="#EDF0F2" strokeDasharray="3 3" vertical={false} /><XAxis dataKey="label" tick={{ fontSize: 11, fill: "#98A2B3" }} axisLine={false} tickLine={false} /><YAxis domain={[40,100]} ticks={[40,55,70,85,100]} tick={{ fontSize: 11, fill: "#98A2B3" }} axisLine={false} tickLine={false} /><Tooltip formatter={(value) => [`${value}%`, "정답률"]} /><Line type="monotone" dataKey="accuracy" stroke="#4C75DD" strokeWidth={3} dot={{ fill: "#8CC0EB", r: 4, strokeWidth: 0 }} animationDuration={650} /></LineChart></ResponsiveContainer></div></section>
    <section className="rounded-card border border-border bg-surface p-4"><h3 className="text-sm font-bold text-muted">세부 분석</h3><dl className="mt-3 divide-y divide-divider text-sm"><Row label="총 시간" value="28분 15초" /><Row label="오답 유형" value="추론형 2문항, 표현형 1문항" /><Row label="시간 초과 문항" value="3, 7번 문항" /></dl></section>
    <Link href={routeBuilders.parent.weakness("독서")} className="flex h-[52px] items-center justify-center rounded-xl border border-[#A9D4F2] bg-surface text-sm font-bold text-[#2F6FA7]">관련 고급분석 보기</Link>
  </div>;
}
